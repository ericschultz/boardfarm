...
class Interact(rootfs_boot.RootFSBootTest):
    ...
    def runTest(self):
        ...
        while True:
            ...
            print("  LAN device:    ssh %s@%s" % (self.config.board.get('lan_username'), self.config.board.get('lan_device')))
            print("  WAN device:    ssh %s@%s" % (self.config.board.get('wan_username') ,self.config.board.get('wan_device')))
